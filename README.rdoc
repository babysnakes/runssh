...
* Migrate to new ConfigFile:
  ...
  2. Add version to the config file (use "VERSION" key as string and not as symbol
     so it won't interfere with the paths)
  3. Create discovery of config version on initialization:
     * If config file is un-versioned abort the application and suggest
       running with <tt>--upgrade-config</tt> argument. 
  4. Create a upgrade-config method for upgrading the configurations file:
...
* Add support for ssh gateway
* Add support for SCP and remote commands.
